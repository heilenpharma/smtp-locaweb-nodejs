...
function checkLimits(email) {
  if (email.subject.length > 998) {
    console.warn(
      "WARNING: título das mensagens não superar 998 caracteres. API vai rejeitar a sua mensagem"
    );
  }

  if (email.body.length > 1048576) {
    console.warn(
      "WARNING: o corpo da mensagem não pode superar 1M. API vai rejeitar a sua mensagem"
    );
  }

  if (email.to.length > 1000) {
    console.warn(
      "WARNING: cada mensagem não pode ter mais de do que 1000 destinatários por minuto"
    );
  }
